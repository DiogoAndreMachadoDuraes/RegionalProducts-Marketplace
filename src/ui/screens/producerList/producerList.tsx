...
export const ProducerList: React.FC = () => {
	...
	const search = () => {
		if (producer !== undefined) {
			const name = Object.values(producer).filter((a) => a.name.toLowerCase().includes(searchTerm.toLowerCase()));
			const tin = Object.values(producer)
							...
							<Col sm={10}>
								...
								>
									...
									>
										...
										<tbody>
											{(producer === undefined || producer === null) && <TableNoResults />}
											{searchTerm === '' && (producer !== undefined || producer !== null)
												? producer?.map((item: Producer, index: number) => {
														return <TableShow item={item} index={index} />;
												  })
												: search()}
										</tbody>
