...
class Rate extends React.Component {
  ...
  async componentDidMount() {
    console.log("Mounting screen Rate...");
    try {
    ...
    try {
      let response = await fetch("http://127.0.0.1:5000/rate", {
        headers: {
      ...
      console.log("Error to get Rate: " + e);
      this.setState({
        isLoading: true,
      });
    }
  ...
  postRate = async () => {
    const { orderId, rate, token } = this.state;
    try {
      ...
      console.log("Error to post Rate: " + e);
